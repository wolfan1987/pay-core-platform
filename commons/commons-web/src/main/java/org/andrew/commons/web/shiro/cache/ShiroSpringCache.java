...
public class ShiroSpringCache<K, V> implements org.apache.shiro.cache.Cache<K, V> {
    ...
    public V get(K key) throws CacheException {
        if (logger.isTraceEnabled()) {
            logger.trace("从cache中获取值 [" + this.cache.getName() + "],key名： [" + key + "]，key类型:" +
                         key.getClass());
        }
    ...
    public V put(K key, V value) throws CacheException {
        if (logger.isTraceEnabled()) {
            logger.trace("设置值到缓存中， [" + this.cache.getName() + "] key名称： [" + key + "] key类型:" +
                         key.getClass());
        }
        V previous = get(key);
        cache.put(key, value);
    ...
    public V remove(K key) throws CacheException {
        if (logger.isTraceEnabled()) {
            logger.trace(
                "从cache中移除指定key的值 [" + this.cache.getName() + "] key： [" + key + "] key类型:" +
                key.getClass());
        }
        V previous = get(key);
        cache.evict(key);

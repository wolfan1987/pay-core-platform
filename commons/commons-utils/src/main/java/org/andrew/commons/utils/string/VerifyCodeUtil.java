...
public class VerifyCodeUtil {
    ...
    public static BufferedImage generateImageCode(
        int type, int length, String excludeString, int width, int height, int interLine,
        boolean randomLocation, Color backColor, Color foreColor, Color lineColor) {
        String textCode = generateTextCode(type, length, excludeString);
        return generateImageCode(
            textCode, width, height, interLine, randomLocation, backColor, foreColor, lineColor);
    }
     ...
     */
    public static BufferedImage generateImageCode(
        String textCode, int width, int height, int interLine, boolean randomLocation,
        ...
        if (interLine > 0) {
            ...
            for (int i = 0; i < interLine; i++) {
                ...
                graphics.drawLine(xaxis, yaxis, x1, y1);
            }
        }
        //字体大小为图片高度的80%
        ...
        switch (type) {
            case TYPE_NUM_ONLY:
                while (index < length) {
                    ...
                    //排除特殊字符
                    if (null == excludeString || excludeString.indexOf(randomNumber + "") < 0) {
                        verifyCode.append(randomNumber);
                        index++;
                    }
                }
                break;
            case TYPE_LETTER_ONLY:
                while (index < length) {
                    ...
                    if ((randomNumber >= 97 || (randomNumber >= 65 && randomNumber <= 90)) &&
                        (null == excludeString || excludeString.indexOf((char) randomNumber) < 0)) {
                        verifyCode.append((char) randomNumber);
                        index++;
                    }
                }
                break;
            ...
            case TYPE_NUM_UPPER:
                while (index < length) {
                    ...
                    if ((randomNumber >= 65 || (randomNumber >= 48 && randomNumber <= 57)) &&
                        (null == excludeString || excludeString.indexOf((char) randomNumber) < 0)) {
                        verifyCode.append((char) randomNumber);
                        index++;
                    }
            ...
            case TYPE_NUM_LOWER:
                while (index < length) {
                    int randomNumber = random.nextInt(123);
                    if ((randomNumber >= 97 || (randomNumber >= 48 && randomNumber <= 57)) &&
                        (null == excludeString || excludeString.indexOf((char) randomNumber) < 0)) {
                        verifyCode.append((char) randomNumber);
                        index++;
                    }
